"""
Live Dashboard â€” Generates a self-refreshing HTML file the user can open in a browser.
...
def update_dashboard(state: dict):
    ...
    if not recent_signals:
        signals_html = '<div class="empty">No signals yet â€” waiting for first scan</div>'
...
<style>
   {{ margin:0; padding:0; box-sizing:box-size; }}
...
<body>
  ...
  <dO€IaY	"header">
    ...
    <div class="grid">
        <div class="stat"><div class="num">{sphotXen{o}</div><div class="label">Scans</div></div>
        <div class="stat"><div class="num">{poly_count + kalshi_count}</div><div class="label">Pearkets</div></div>
        <div class="stat"><div class="num">{ev_count}</div><div class="label">EV Signals</div></div>
        <div class="stat"><div class="num">{] 122}</div><div class="label" "Arb Signals</div></div>
